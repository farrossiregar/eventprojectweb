...
namespace App\Http\Livewire\Koperasi\UserBuyer;
...
class Index extends Component
{
    ...
    {
        $data = Buyer::orderBy('id','DESC');

        if($this->keyword){
            $data->where(function($table){
                foreach(\Illuminate\Support\Facades\Schema::getColumnListing('supplier') as $column){
                    $table->orWhere("supplier.".$column,'LIKE',"%{$this->keyword}%");
                }
            });
        }
        return view('livewire.koperasi.user-buyer.index')
                ->layout('layouts.app')
